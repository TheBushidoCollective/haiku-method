...
- **Proposed default**: 30 days from submission, auto-deleted. The derived fix-loop state (PR number, iteration count, CI results, bot comments) can be retained on a separate, longer schedule (90 days) since it contains no user-originated raw session content. Deletion requests from the user are honored within 7 days via the `report/:id` status page deletion link or the `oss@gigsmart.com` email fallback. **The retention default and the 7-day deletion SLA cover GigSmart's Cloud Run storage only.** The bundle copy that the fix agent transmits to the Anthropic API during processing sits under Anthropic's API data handling policy and is irreversible from GigSmart's side: GigSmart cannot compel or confirm deletion from Anthropic's inference infrastructure. The consent step and the privacy policy update must surface this asymmetry in user terms; this is captured as the "Irreversible Secondary Transmission to Anthropic API" entry in `risk-inventory.md`.
- **Rationale**: Most fix loops close within minutes to hours; 30 days covers the long tail of issues that require human pickup after the iteration cap is hit. The 30-day window aligns with common industry practice for diagnostic artifact retention (e.g., Sentry's default event retention). The privacy principles (unit-04) name 30 days as the proposed default explicitly and provide the rationale for separating raw-bundle retention from derived-state retention. The Anthropic-side carve-out is a strategic disclosure obligation, not a retention number GigSmart can set — it is named here so downstream stages do not inadvertently treat the 30-day/7-day SLAs as end-to-end deletion guarantees.
- **Source**: `stages/inception/artifacts/privacy-and-data-handling-principles.md` (unit-04), "Open Questions — Retention duration" section; `stages/inception/artifacts/privacy-and-data-handling-principles.md` (unit-04), "Retention and disclosure — Deletion-request mechanism" section; `stages/inception/artifacts/risk-inventory.md` (unit-05), "Risk: Irreversible Secondary Transmission to Anthropic API".
