...
### Risk: Irreversible Secondary Transmission to Anthropic API

- **Severity**: high
- **Detection signal**: A user-submitted deletion request for their session bundle is acknowledged by GigSmart, and the bundle is deleted from Cloud Run storage within the disclosed SLA — but the user, upon following up, learns there is no GigSmart-controllable mechanism to confirm or compel corresponding deletion from Anthropic's inference infrastructure. A second detection signal: any privacy or regulatory inquiry (e.g., GDPR right-to-erasure response) where GigSmart cannot fulfill the request end-to-end because the data has already been consumed by the Anthropic API.
- **Mitigation owner**: product

The session bundle, once transmitted from the GigSmart Cloud Run service to the Anthropic API as part of the fix-agent prompt context, leaves GigSmart's controllable surface. GigSmart can honor a user's deletion request for the bundle stored on Cloud Run (within the proposed 7-day SLA in unit-06), but Anthropic's API data handling — what is retained, for how long, and under what training-vs-inference data agreement — sits inside Anthropic's infrastructure and Anthropic's terms. GigSmart cannot guarantee deletion from Anthropic on the user's behalf, cannot independently audit what Anthropic retains, and cannot offer the user end-to-end deletion certainty for the data that already crossed the API boundary. This is materially different from the storage risks the rest of this inventory covers: GigSmart's Cloud Run retention is bounded and deletable; Anthropic's consumption is irreversible from GigSmart's side. The user-outcome implications are concrete: the consent step must name Anthropic as a data recipient at the moment of submission (not in a privacy policy footnote), the privacy policy update must describe the secondary transmission and the limit of GigSmart's deletion authority, and any GDPR right-to-erasure response GigSmart returns must explicitly carve out the Anthropic-side data the user must pursue under Anthropic's own terms. The severity is rated high, equal to the "Anthropic API Single-Vendor Dependency" availability/cost risk and adjacent to the critical "Privacy Policy Compliance Gap" risk because under-disclosure here lands in the same compliance-exposure space.
