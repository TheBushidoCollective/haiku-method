...
|---|---|---|
| **Disclosure shown and consent confirmed before any transmission** | Plugin conversation displays a structural summary listing the data classes the scrubber will strip (e.g., "3 credential-shaped strings, 1 home path, 0 emails detected") and the destination of the bundle (GigSmart Cloud Run + Anthropic API), and the user must take an explicit confirm action before the POST happens. | The user reaches the submit step in `/haiku:report`. The skill blocks on a confirmation prompt that names what is being sent, where, and what was scrubbed; the POST does not fire until the user confirms. If the user declines or abandons the prompt, no data leaves the machine. |
...
**Note on scrubber correctness as an internal quality gate.** The privacy principles (unit-04) and the risk inventory (unit-05, "Risk: Scrubber False Negatives") commit the scrubber to stripping known-sensitive data classes before the bundle is POSTed. That commitment is an **internal correctness invariant** — it is verifiable by code review, unit tests against fixture credentials, and a backend post-transmission audit signal — but it is **not a user-observable acceptance criterion**, because a scrubbing false negative is invisible to the user at submission time. To keep acceptance user-observable, this artifact treats scrubber correctness as an internal quality gate (owned by the design and development stages and enforced by tests) and pairs it with the user-observable disclosure-and-consent end state above. The user's check at submission time is "did the system show me what would be sent and ask me before sending it?" — not "did the scrubber catch every secret?", which the user cannot independently verify.
