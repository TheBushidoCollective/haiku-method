...
| `unit-03-symlink-toctou-and-csrf` | `safeMkdirAndRename` helper, three-layer CSRF (`http/csrf.ts`), feedback body sanitizer (`http/feedback-sanitize.ts`), operator-only baseline-corrupt gate | V-04 (partial — single-shot close, full O_NOFOLLOW deferred), V-08 (Layers 1-3 + Layer 4 per-IP rate-limit landed via `@fastify/rate-limit` in tunnel mode; per-route differentiation + per-session keying still deferred — see §4 R-3 "Remaining gaps"), V-10, V-11 | `06cbb625c`; suite_at_commit reported in fix commits as `OK` (V-04, V-08, V-10, V-11 commits); 11/11 gates passed at `2026-05-03T09:00:39Z` (after re-verification against the new `csrf.ts` and `feedback-sanitize.ts` files where the fix code actually lives) |
