...
| **V-07** | Medium | `HAIKU_UPLOAD_MAX_BYTES` parsing has no upper bound; misconfigured value lets one upload exhaust disk + RAM + drift-gate sync SHA. Symmetric MCP-tool path (`haiku_human_write` `content`) was uncapped, leaving the same DoS reachable via the MCP entry point. | `unit-01-upload-content-validation` (SPA), `unit-04-threat-model-and-assessments` (MCP-path symmetry, FB-30) | `grep -qE 'MAX_UPLOAD_BYTES_HARD_CAP\|Math\.min.*HAIKU_UPLOAD_MAX_BYTES\|uploadHardCap' packages/haiku/src/http/upload-routes.ts` (gate `v07-upload-max-bytes-hard-cap`) **plus** `v07-oversize-clamp-test-named` **plus** `grep -qE 'MAX_CONTENT_BYTES_HARD_CAP\|HUMAN_WRITE_MAX_CONTENT_BYTES_HARD_CAP' packages/haiku/src/tools/orchestrator/haiku_human_write.ts` (gate `v07-mcp-content-bytes-hard-cap`) **plus** `v07-mcp-content-too-large-test-named` (matches "V-07 (MCP)" in `packages/haiku/test/haiku-human-write.test.mjs`) | SPA path: commit `f83f45fe5`, run `2026-05-03T09:00:39Z`, exit_code=0 (both gates). `MAX_UPLOAD_BYTES_HARD_CAP = 50 MiB`; `Math.min` clamp + `haiku.upload.cap_clamped` telemetry on misconfig (commit `3867608a6`). MCP path: `MAX_CONTENT_BYTES_HARD_CAP = 50 MiB` mirrors the SPA cap (FB-30); pre-decode short-circuit on raw string length avoids allocating a multi-gigabyte Buffer; post-decode authoritative gate rejects with `content_too_large` before any disk write or audit append. Three regression tests cover oversize utf-8, oversize base64, and at-cap acceptance. | Drift-gate sync SHA on tracked files >50 MiB still blocks the workflow tick if anyone ever bumps either cap. Recommendation (not yet implemented): skip drift-gate hashing for tracked files larger than a configurable limit and emit `haiku.drift.skipped_large` telemetry — folded into the rate-limit / DoS hardening residual risk. The two caps MUST move together (block-comment in both source files cites this contract); a follow-up could centralise into a shared constant once the http→tools layering question (FB-23) is settled. |
...
| `unit-04-threat-model-and-assessments` (this unit) | THREAT-MODEL.md + ASSESSMENTS.md synthesis; V-07 MCP-path symmetry (`MAX_CONTENT_BYTES_HARD_CAP` in `haiku_human_write.ts`, FB-30) | V-07 MCP-path closure (D-1' threat row added in §3.5) | (gates evaluated by workflow engine on advance) |
