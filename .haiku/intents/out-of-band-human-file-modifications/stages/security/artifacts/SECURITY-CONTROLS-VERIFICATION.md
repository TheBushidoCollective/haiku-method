...
| R-3 — Rate limiting: **per-IP aggregate cap LANDED** via `@fastify/rate-limit` plugin-wide registration in tunnel mode (`packages/haiku/src/http.ts:228-243`, default `max=60` per `60_000` ms, IP-keyed via `req.ip` with intentional `trustProxy=false`; full characterization in THREAT-MODEL.md §6.6). **Remaining (deferred)**: per-token/per-session keying for V-09 isolation, per-route differentiation for upload-vs-feedback profiles, WebSocket frame-stream limiting, slowloris connection-timeout (`connectionTimeout`/`requestTimeout` on the Fastify factory), cumulative-bytes-per-intent quota | FB-08 | Low / Medium for the LANDED per-IP layer; Medium for the deferred per-session/per-route gaps; Medium-High → Medium for slowloris pending the connection-timeout fix | Direct CSRF exploit closed by 3-layer defense; the present per-IP limiter caps the request-rate flood vector against V-08 Layer 4 + D-3 + D-4; remaining deferrals are per-session keying (V-09 abuse isolation) and connection-level slowloris timeout (independent of request-rate) — see ASSESSMENTS.md §4 R-3 for the full present-coverage / remaining-gaps split |
