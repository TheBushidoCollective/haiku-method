...
export function QuestionPage({ session, sessionId, wsRef }: Props) {
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		const answers: QuestionAnswer[] = questions.map((q, i) => {
			...
			const selectedOptions = Array.from(selected).filter(
				(o) => o !== "__other__",
			)
			...
			return {
				...
				selectedOptions,
				otherText: hasOther
					? (otherTexts.get(i) ?? "").trim() || undefined
					: undefined,
			}
		})

		const annotations =
			inlineCommentsRef.current.length > 0
				? {
		...
		<>
				...
				<Card>
					...
					<InlineComments
						...
						onCommentsChange={(comments) => {
							inlineCommentsRef.current = comments
						}}
					/>
				</Card>
			)}
				...
				<Card>
					...
					{imageUrls.length >= 2
						? // Pair images side-by-side
							Array.from(
								{ length: Math.ceil(imageUrls.length / 2) },
								(_, pairIdx) => {
			...
			<form onSubmit={handleSubmit} noValidate>
						...
						<Card key={qIdx}>
							<fieldset>
								<legend className="text-base font-semibold mb-1 text-stone-900 dark:text-stone-100">
									<MarkdownViewer id={`q-${qIdx}-question`}>
										{q.question}
									</MarkdownViewer>
								</legend>
								{q.header && (
									<div className="text-sm text-stone-500 dark:text-stone-400 mb-3">
										<MarkdownViewer id={`q-${qIdx}-header`}>
											{q.header}
										</MarkdownViewer>
									</div>
								)}
								<div className="space-y-2">
									{q.options.map((option) => (
										...
										>
											<input
												...
												onChange={(e) =>
													handleOptionChange(qIdx, option, e.target.checked)
												}
									...
									<label className="flex items-center gap-3 p-2 rounded-lg hover:bg-stone-50 dark:hover:bg-stone-800 cursor-pointer transition-colors">
										<input
											...
											onChange={(e) =>
												handleOptionChange(qIdx, "__other__", e.target.checked)
											}
											className="w-4 h-4 text-teal-600 focus:ring-2 focus:ring-teal-500"
											aria-label="Other"
											disabled={submitting}
				...
				<button
					...
					className="w-full px-6 py-3 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-lg transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-stone-900 disabled:bg-green-300 disabled:text-green-800 dark:disabled:bg-green-900/40 dark:disabled:text-green-200 disabled:cursor-not-allowed"
				>
				...
				{result && (
					...
					>
						...
						{result.success && (
							<p className="text-sm mt-1">You can close this tab.</p>
						)}
					</div>
				)}
			</form>
