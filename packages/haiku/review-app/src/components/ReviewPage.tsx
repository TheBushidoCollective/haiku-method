...
function loadDraft(sessionId: string): ReviewDraft {
	try {
		...
		return {
			generalText:
				typeof parsed.generalText === "string" ? parsed.generalText : "",
			generalComments,
		}
	} catch {
...
export function ReviewPage({ session, sessionId, wsRef }: Props) {
	...
	const activeStage =
		stageNames.find((s) => stageStates[s]?.status === "active") ??
		stageNames[0] ??
		null
	...
	const gateType = (
		session.gate_type?.includes("ask")
			? "ask"
			: session.gate_type?.includes("external")
				? "external"
				: "auto"
	) as "ask" | "external" | "auto"
	...
	// Sidebar tab state
	const [sidebarTab, setSidebarTab] = useState<"comments" | "feedback">(
		"comments",
	)
	...
	const [allInlineComments, setAllInlineComments] = useState<
		InlineCommentEntry[]
	>([])
	...
	const [generalComments, setGeneralComments] = useState<SidebarComment[]>(
		initialDraft.generalComments,
	)
	const [generalText, setGeneralText] = useState<string>(
		initialDraft.generalText,
	)
	...
	const handleAddGeneral = useCallback((comment: string) => {
		...
		setGeneralComments((prev) => [
			...prev,
			{
				...
				text: "",
				comment,
				id: `general-${generalCounter.current}-${Date.now()}`,
			},
		])
	}, [])
		...
		if (inlineMatch) {
			...
			if (inlineMatch.highlightEl?.parentNode) {
				...
				while (inlineMatch.highlightEl.firstChild) {
					parent.insertBefore(
						inlineMatch.highlightEl.firstChild,
						inlineMatch.highlightEl,
					)
				}
		...
		if (inlineMatch) {
			setAllInlineComments((prev) =>
				prev.map((c) => {
					...
					if (c.highlightEl) {
						c.highlightEl.setAttribute(
							"aria-label",
							`Commented text: ${newComment || "(no comment)"}`,
						)
					}
	...
	const handleInlineCommentsChange = useCallback(
...
function IntentReview({
	...
	const intent =
		...
		({
			...
			sections: [],
			rawContent: "",
		} as unknown as NonNullable<SessionData["intent"]>)
