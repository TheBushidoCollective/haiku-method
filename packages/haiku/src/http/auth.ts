// http/auth.ts — Tunnel JWT extraction + per-mutation auth + V-08 CSRF.
//
...
//
// V-08 CSRF defence-in-depth lives in this same module — `auth + csrf
// are conceptually one boundary` (the chokepoint that answers "is this
// request allowed to mutate?"). Keeping them in one module eliminates
// the historical circular import (csrf.ts ⇄ auth.ts) and the dead
// re-export shim that existed only to satisfy quality-gate grep
// predicates. The three layers and the `/api/csrf-nonce` endpoint
// registration live below the auth surface, in source order:
//
//   ── Tunnel JWT extraction + per-mutation auth ──
//   ── V-08 CSRF: configuration + nonce store ──
//   ── V-08 CSRF: layered preHandler + route registration ──
 ...
 *  route. Lives on the auth surface (this file) because auth + csrf are
 *  the same boundary. */
...
/** Alias retained for callers that imported the historical name from
 *  the auth surface re-export shim. The implementation IS this file's
 *  `csrfPreHandler`; both names dispatch to the same function. */
export const requireCsrfNonce = csrfPreHandler

/** Alias retained for callers that imported the historical name from
 *  the auth surface re-export shim. */
export const checkOrigin = isOriginAllowed
