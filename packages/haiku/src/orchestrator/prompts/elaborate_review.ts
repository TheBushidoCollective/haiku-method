...
export default definePromptBuilder((ctx) => {
	...
	sections.push("```")
	sections.push(`You are the elaboration verifier for intent ${intentSlug}, stage ${stage}.`)
	sections.push("")
	sections.push(`Your single job: read three files and decide whether the captured conversation engaged substantively with *this* intent's goals as they bear on *this* stage's scope.`)
	sections.push("")
	...
	sections.push("")
	sections.push(`Pass criteria (ALL must be true):`)
	sections.push(`- The conversation references specific content from the intent body, not just the FM.`)
	sections.push(`- The questions surfaced are tied to ambiguities in *this* intent on *this* stage's scope. Generic questions ("what do you want?") fail.`)
	sections.push(`- The agreement captured at the end is concrete enough that downstream unit decomposition could anchor on it.`)
	sections.push(`- The conversation surfaces at least one decision point or clarification, not just acknowledgment.`)
	sections.push("")
	sections.push(`Fail signals:`)
	...
	sections.push(`- No reference to specific intent content (mobile, desktop, integrations, named features, etc.).`)
	sections.push("")
	sections.push(`On pass: call \`haiku_stage_elaboration_seal\` with { intent: "${intentSlug}", stage: "${stage}" }. The tool stamps \`verified_at\` on the artifact's frontmatter.`)
	sections.push("")
	sections.push(`On fail: do NOT call seal. Return a structured response with the specific gaps the outer agent must address — quote the lines from the artifact that are thin, name what's missing, and suggest what the next conversation turn should cover. The outer agent will overwrite the artifact and re-verify.`)
	sections.push("```")
