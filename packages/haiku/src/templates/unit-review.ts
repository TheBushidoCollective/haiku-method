...
export function renderUnitReview(
	...
	const techSpec =
		findSection("Technical Spec") ||
		findSection("Technical Specification") ||
		findSection("Implementation")
	const domainEntities =
		findSection("Domain Entities") || findSection("Entities")
	...
	if (description)
		combinedSpecMarkdown += `## Description\n\n${description}\n\n`
	if (techSpec) combinedSpecMarkdown += `## Technical Spec\n\n${techSpec}\n\n`
	if (domainEntities)
		combinedSpecMarkdown += `## Domain Entities\n\n${domainEntities}\n\n`

	const specContent = `
    ${renderBreadcrumb([{ label: intent.title }, { label: unit.title }])}
  ...
  `)
	...
	const boundaries =
		findSection("Boundaries") ||
		findSection("Out of Scope") ||
		findSection("NOT in scope")
