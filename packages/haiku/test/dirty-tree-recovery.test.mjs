...
await test(
	"intent.md uncommitted at start_stage time is auto-committed before checkout",
	async () => {
...
await test(
	"the canary text from the dirty edit lands in git history (proof the pre-stage commit captured it)",
	async () => {
		const { root, git, studio, cleanup } = makeRepoWithStudio()
		try {
			process.chdir(root)
