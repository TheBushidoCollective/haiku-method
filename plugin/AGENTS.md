...
This file is the context for harnesses that read `AGENTS.md` (OpenAI Codex, OpenCode, GitHub Copilot CLI). The H·AI·K·U MCP server is what drives everything — the slash-command skills (`/haiku:haiku-start`, `/haiku:haiku-pickup`) are Claude-specific sugar. Outside Claude, you drive the workflow by calling the MCP tools directly.
