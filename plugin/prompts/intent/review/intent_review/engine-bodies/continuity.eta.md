...
- Per-orphan or per-broken-reference: `haiku_feedback({ intent: "<slug>", origin: "engine-review", author: "continuity", severity: "high", source_ref: "continuity:<path>", target_unit: <best guess or null>, target_invalidates: [] })` (intent scope — omit `stage`; a broken reference is a concrete defect, so `high` — bump to `blocker` if it makes the deliverable non-functional)
