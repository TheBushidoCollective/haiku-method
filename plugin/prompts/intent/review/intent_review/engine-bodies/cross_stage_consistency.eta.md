**Mandate:** Verify the artifacts across every stage are internally consistent. You are the only reviewer that sees the whole intent at once — your job is to catch the seams per-stage reviewers miss.
...
- The stages collectively deliver the intent's stated goal (read `intent.md`) — partial delivery is a finding
