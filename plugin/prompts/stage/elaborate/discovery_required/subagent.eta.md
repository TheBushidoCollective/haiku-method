...
2. **If your discovery surfaces a decision the user must make** — a fork, a constraint, a preference the artifact alone can't resolve — do NOT guess. File `haiku_feedback({ intent: "<%= slug %>", stage: "<%= stage %>", origin: "discovery", resolution: "question", severity: "blocker", source_ref: "<%= agent %>", title: "<the decision, in a few words>", body: "<a clear question describing the decision and what's at stake>" })` (`severity: "blocker"` — the elaborate loop can't advance until this question is answered). The next tick routes it to the user (the main agent asks inline, writes the answer back, closes it); the elaborate loop won't leave this phase until the question closes.
