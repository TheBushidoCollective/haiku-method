...
4. **Close — end your turn with exactly ONE:**
   - **Success:** `haiku_unit_advance_hat { intent: "<%= slug %>", unit: "<%= unit %>", message: "<handoff baton>" }`. The `message` is REQUIRED — it's the baton the next hat reads: state what you did, what you concluded, and what the next hat needs to pick up cleanly (decisions made, open questions, and the files you produced — reference them by path, don't re-paste their contents). Write it for the hat that reads it next, not as a status line. If `iterations[]` is empty on the first call, prefix with `haiku_unit_start { intent: "<%= slug %>", unit: "<%= unit %>" }` so the engine stamps the in-flight iteration.
   - **Block:** `haiku_unit_reject_hat { intent: "<%= slug %>", unit: "<%= unit %>", message: "<objection + what must change>" }`. The `message` is REQUIRED — name the specific objection AND what must change to pass (e.g. "touch targets <44px on mobile — bump to 44px min and re-check the nav row"), so the re-run hat fixes exactly that instead of re-discovering it.
