...
Every mutating op also accepts an optional `reason` — free-text prose explaining *why* you're reaching for the admin tool (the cause that led here, not what the op does). It's surfaced verbatim at the top of the confirmation picker so the user has the context to approve or cancel. Always pass it; the user is being asked to authorize a workflow-engine bypass and deserves to know what they're unblocking.
...
haiku_debug({
  ...
  stage: "<target-stage>",
  reason: "Every unit in <target-stage> moved through every hat, but the cursor won't advance — approvals never got stamped. Force-completing to unblock.",
})
```
