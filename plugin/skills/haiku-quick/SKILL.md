...
2. **Create the intent** with `haiku_intent_create`:
   ...
   - **`studio_candidates` (REQUIRED)**: the 2–4 studios that best fit, from `haiku_studio_list`. Fetch the list, pick the closest matches, pass their canonical names. The tool rejects the call without it (and tells you to fetch the list and retry).
...
4. **After studio is selected**, the engine routes to `select_mode`. Call `haiku_select_mode { intent: "<slug>", options: ["quick"] }` — passing `options: ["quick"]` locks the mode without showing the picker (this is the only thing that distinguishes `/haiku:haiku-quick` from `/haiku:haiku-start`).
...
- If the task needs multiple stages, stop and suggest `/haiku:haiku-start` instead — don't cram it into a single stage.
