...
1. **Identify the intent.** If no intent slug is known from context, call `haiku_intent_list` first. If multiple are active and the user didn't name one, ask which intent before proceeding — the same discovery preflight `/haiku:haiku-reset-intent` does.
