...
**Per-stage delivery PR (discrete / discrete-hybrid).** Independent of the cursor's filesystem-driven walk (Rule 1 — the cursor reads no git topology): the git layer opens a **draft** PR for the stage at stage start (`workflowStartStage` → `openStageDraftPullRequest`, branch `haiku/<slug>/<stage>` → base `haiku/<slug>/main`), recorded in the `stage_prs` map on intent.md FM. It is where the stage's runtime-verification proof (gitignored locally) gets uploaded, and the stage gate flips it draft→ready (`haiku_await_gate` external-review path → `markPullRequestReady`) rather than opening a second PR; merging it is the approval signal. `discrete` opens one per stage; `discrete-hybrid` opens one only for stages whose `review:` gate is/includes `external` (`stageRequiresExternalReview`) — the continuous stages keep their work on the intent-main PR. continuous / autopilot / quick open no per-stage PR — they keep the single intent-main draft PR (opened at `intent_create`) and proof uploads there. This is a delivery-shape detail under the git/handler layer, not a phase the cursor derives.
