...
| **Review agent (intent-tier)** | `studios/<studio>/intent-review-agents/<agent>.md` | ONE intent-close integration lens. Fires once at intent close, opt-out via `intent_completion_review: false` on intent FM | Per-stage scope (that's the stage-tier review-agent); modifying earlier-stage artifacts (file feedback instead) |
