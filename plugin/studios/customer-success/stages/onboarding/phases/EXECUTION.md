...
5. **Fix loop (if any feedback opens)** — `fix_hats: classifier → onboarding-lead → feedback-assessor` dispatches per finding. The classifier routes the FB to the right unit or stage; `onboarding-lead` is the implementer (re-defining a milestone or re-sequencing the plan); the assessor independently decides closure.

6. **Gate** — The stage's gate is `ask`. The user reviews the onboarding plan and validates readiness before the workflow advances to `adoption`.
