...
5. **Fix loop (if any feedback opens)** — `fix_hats: classifier → strategist → feedback-assessor` dispatches per finding. The classifier routes the FB to the right unit or stage; `strategist` is the implementer (re-framing or pointing at the evidence gap); the assessor independently decides closure.

6. **Gate** — The stage's gate is `auto`. Once verifier hats advance and review agents are clean, the workflow engine merges and moves on without human approval. The landscape stage is structurally upstream of where most strategic-decision risk lives; gating here would add friction without catching the failures that actually matter.
