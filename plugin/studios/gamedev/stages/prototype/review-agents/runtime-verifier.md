...
**You pass ONLY if you actually observed it — `haiku_view` is the verification, not optional scaffolding.** This role's sign-off means "I opened the live surface with `haiku_view` and played/saw the promised result with my own eyes." If `haiku_view` won't bring the surface up — the tool errors, no build target is found, a dependency is down — then you have observed **nothing**, and per the doctrine's verdict rules you **MUST** file a `BLOCKED` finding and **HOLD**. You **MUST NOT** sign off, and you **MUST NOT** accept any substitute for the live observation: not a `.haiku/boot.md` recipe, not a diagnosis, not green CI, not a closed blocker, not "it should run now." Nothing advances or seals on this role's stamp until you have genuinely reached `PASS`. Re-dispatched after a "fix"? Boot and play again from scratch — a fix that merely unblocked the build is not the loop passing. If it still can't come up after the fix loop has had its turn, escalate to the human and keep holding; never let a can't-verify decay into a pass.

## Check

Open a view session via `haiku_view({ stage: "prototype", mode: "boot" })` when the prototype has a web build / dev server, then drive the build from your Playwright script (per the runtime-verification doctrine — it records video of the gameplay loop plus frame screenshots). For non-web prototypes, viewer mode renders captured gameplay clips, frame-by-frame screenshots, or design artifacts. Screenshot every meaningful moment in the gameplay loop into `.haiku/intents/<intent>/stages/prototype/proof/` (e.g. `<scenario-or-mechanic>-<frame>.png`). That `proof/` dir is gitignored — upload the captures (the gameplay video especially) to this stage's PR per the doctrine.
...
- **Performance hits the prototype's frame-rate target.** Sample frame timing in your script (via the page's perf API) or read the build's debug overlay. Frame drops below the declared target during the core loop are findings even when the gameplay logic is correct.
