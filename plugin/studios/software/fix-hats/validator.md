...
- If the fix is incomplete or wrong: call `haiku_feedback_reject_hat { intent, feedback_id, reason: "<what is missing>" }` (omit `stage`). The FSM bumps the bolt counter and re-dispatches the prior fix-hat, up to the bolt cap before escalation.
