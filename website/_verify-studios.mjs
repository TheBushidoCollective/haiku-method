...
for (const entry of list) {
	...
	const report = await page.evaluate(() => {
		...
		const haveHats = stages.every(
			(s) => s.querySelectorAll(".hat-circle, .hat").length > 0,
		)
		const haveReviewAgents = stages.every(
			(s) =>
				s.querySelectorAll(".review-agent, .ra").length > 0 ||
				s.querySelector(".phase-review") !== null,
		)
		const names = stages.map((s) =>
			s.querySelector("header h2")?.textContent.trim(),
		)
		const hasAnyArtifactIn = stages.some(
			(s) => s.querySelectorAll(".artifacts.in .artifact").length > 0,
		)
		const hasAnyArtifactOut = stages.some(
			(s) => s.querySelectorAll(".artifacts.out .artifact").length > 0,
		)
		...
		return {
			...
			hasAnyArtifactOut,
			activeName,
			auxHasContent: auxText.length > 0,
		}
	})

	if (report.count !== entry.stageCount) {
		failures.push(
			`${entry.dir}: expected ${entry.stageCount} stages, got ${report.count}`,
		)
	}
	if (report.activeName !== entry.dir) {
		failures.push(
			`${entry.dir}: picker did not set active to ${entry.dir} (got ${report.activeName})`,
		)
	}
	if (!report.haveHats)
		failures.push(`${entry.dir}: one or more stages has zero hat chips`)
	if (!report.hasAnyArtifactOut)
		failures.push(
			`${entry.dir}: no output artifacts anywhere — every stage missing outputs?`,
		)
	...
	if (report.count > 1) {
		const nonFirstHasInputs = await page.evaluate(() => {
			...
			return stages
				.slice(1)
				.some((s) => s.querySelectorAll(".artifacts.in .artifact").length > 0)
		})
		if (!nonFirstHasInputs)
			failures.push(
				`${entry.dir}: no downstream stage has inputs — cross-stage wiring looks broken`,
			)
	}
...
async function clickAtPath(path, label) {
	...
	const opened = await page.evaluate(({ stageIdx, within }) => {
		...
		const visible =
			backdrop && !backdrop.hidden && backdrop.classList.contains("open")
		const contentLen = body ? body.textContent.trim().length : 0
		return { ok: visible && contentLen > 50, visible, contentLen }
	}, path)
	if (!opened.ok) {
		failures.push(
			`${label}: modal did not open with content (${JSON.stringify(opened)})`,
		)
	} else {
...
await clickAtPath(
	{ stageIdx: 0, within: ".hat-circle, .hat" },
	"first-stage hat",
)
await clickAtPath(
	{ stageIdx: 0, within: ".artifacts.out .artifact" },
	"first-stage output artifact",
)
await clickAtPath(
	{ stageIdx: 1, within: ".artifacts.in .artifact" },
	"second-stage input artifact",
)
...
const auxSnapshot = async () => {
	return page.evaluate(
		() => document.getElementById("aux-content")?.textContent ?? "",
	)
}
...
if (!auxSoftware || !auxLegal) {
	...
	failures.push(
		"aux cards: identical across software + legal (studio swap not affecting them?)",
	)
} else {
	console.log(
		`  aux cards swap software↔legal: differing content ✓ (${auxSoftware.length} vs ${auxLegal.length} chars)`,
	)
}
...
if (stage1Software && stage1Ideation && stage1Software === stage1Ideation) {
	...
	console.log(
		"  note: pre-intent card text identical across software+ideation (expected if it's studio-agnostic copy)",
	)
