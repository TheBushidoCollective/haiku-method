...
export function UnitReview({ session, baseUrl }: Props) {
	...
	const criteria = (session.criteria ?? []) as Array<{
		text: string
		checked: boolean
	}>
	...
	const title =
		(targetUnit.title as string) ?? (targetUnit.slug as string) ?? "Unit"
	const status = (targetUnit.status as string) ?? "pending"
	const sections =
		(targetUnit.sections as Array<{ heading: string; content: string }>) ?? []
	...
	return (
		<div className="mx-auto max-w-3xl">
			...
			{/* Mockups */}
			{session.intent_mockups &&
				(session.intent_mockups as Array<Record<string, unknown>>).length >
					0 && (
					<section className="mb-8" id="section-mockups">
						...
						<div className="grid grid-cols-2 gap-4">
							{(session.intent_mockups as Array<Record<string, unknown>>).map(
								(m, i) => {
