	...
	for (const key of keys) {
		...
		items.push(
			<div
				key={fullPath}
				data-path={fullPath}
				className={`whitespace-nowrap demo-animate-in ${isDir ? "text-stone-500" : "text-stone-400"} ${isHighlighted ? "demo-highlight-glow" : ""} ${hasArtifact ? "cursor-pointer hover:text-teal-400 transition-colors" : ""}`}
				style={{ paddingLeft: `${depth * 16 + 4}px` }}
				onClick={hasArtifact ? () => onFileClick(fullPath) : undefined}
			...
			>
				...
				{key}
				{hasArtifact && (
					<span className="ml-1.5 inline-block text-[10px] text-teal-600 opacity-60">
						&#8599;
					</span>
				)}
			</div>,
...
function ArtifactViewer({
	...
	if (isMarkdown && content.startsWith("---")) {
		...
		if (endIndex !== -1) {
			...
			displayContent = body
			// We'll show frontmatter separately
			return (
				<div className="flex h-full flex-col">
					{/* Header */}
					<div className="flex shrink-0 items-center justify-between border-b border-stone-700 bg-stone-900 px-4 py-3">
						<div className="min-w-0 flex-1">
							<div className="truncate font-mono text-sm font-semibold text-teal-400">
								{fileName}
							</div>
						...
						<button
							type="button"
							onClick={onClose}
							className="ml-3 flex h-7 w-7 shrink-0 cursor-pointer items-center justify-center rounded-md border border-stone-700 bg-stone-800 text-stone-400 transition-colors hover:border-teal-400 hover:text-teal-400"
					...
					{/* Frontmatter */}
					<div className="shrink-0 border-b border-stone-800 bg-stone-950/50 px-4 py-2">
						<pre className="font-mono text-[11px] leading-relaxed text-stone-500">
							{frontmatter}
						</pre>
					</div>

					{/* Body */}
					<div className="flex-1 overflow-y-auto px-5 py-4 [&::-webkit-scrollbar-thumb]:rounded-sm [&::-webkit-scrollbar-thumb]:bg-stone-700 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar]:w-1.5">
						<div className="prose prose-invert prose-sm max-w-none prose-headings:text-stone-200 prose-p:text-stone-300 prose-li:text-stone-300 prose-strong:text-stone-200 prose-code:text-teal-400 prose-code:bg-stone-800 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-code:text-xs prose-pre:bg-stone-950 prose-pre:border prose-pre:border-stone-800 prose-a:text-teal-400 prose-th:text-stone-400 prose-td:text-stone-400">
			...
			<div className="flex-1 overflow-y-auto px-5 py-4 [&::-webkit-scrollbar-thumb]:rounded-sm [&::-webkit-scrollbar-thumb]:bg-stone-700 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar]:w-1.5">
				{isJson ? (
					<pre className="font-mono text-xs leading-relaxed text-stone-300">
						{(() => {
							try {
								return JSON.stringify(JSON.parse(displayContent), null, 2)
							} catch {
...
export function DemoClient({
	...
	const openFile = useCallback(
		(filePath: string) => {
			if (artifacts && filePath in artifacts) {
		...
		<div className="fixed inset-0 z-50 flex flex-col bg-stone-950 font-sans text-stone-200">
			...
			{viewingFile && artifacts && viewingFile in artifacts && (
				<div className="fixed inset-0 z-[95] flex">
					...
					/>
					{/* Panel */}
					<div className="flex w-[560px] max-w-[85vw] flex-col border-l border-stone-700 bg-stone-900 shadow-[-8px_0_32px_rgba(0,0,0,0.4)] demo-animate-in">
						<ArtifactViewer
							filePath={viewingFile}
							content={artifacts[viewingFile]}
							onClose={closeFile}
						/>
					</div>
				</div>
