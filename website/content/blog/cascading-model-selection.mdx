...
Each layer exists because each layer has a different scope of knowledge about what the work needs.

<Grid>
  <Card eyebrow="Studio" title="Lifecycle default" accent="info">
    "Most work in this lifecycle is Sonnet-grade." Sensible baseline for the whole pipeline.
  </Card>
  <Card eyebrow="Stage" title="Phase override" accent="info">
    "But review phases need Opus for adversarial reasoning." Stage knows something the studio doesn't.
  </Card>
  <Card eyebrow="Hat" title="Role override" accent="info">
    "But the builder hat can usually drop to Haiku for scaffolding." Role knows something the stage doesn't.
  </Card>
  <Card eyebrow="Unit" title="Work-item override" accent="good">
    "But *this particular* unit involves a consensus algorithm, and I want Opus regardless." Unit wins.
  </Card>
</Grid>
...
<Grid>
  ...
  <Card eyebrow="Machine-readable" title="Scriptable from outside" accent="info">
    Tooling that lives outside the harness can read the per-unit model assignments too — so dashboards, audits, and your own cost reports can act on the same decisions.
  </Card>
</Grid>

## A security note

There's a real attack surface here. Frontmatter gets read from `.md` files, and `.md` files get edited by agents. If an attacker could sneak `model: opus` or something stranger into a frontmatter block via prompt injection, they could drive up cost or destabilize a run.

<Callout tone="warn" title="What stops the obvious abuse">
The cascade resolution runs input sanitization on every `model:` value — unknown strings get rejected, not passed through to the spawn call. It's the kind of thing you don't notice when it works, and the kind of thing that eats an incident response when it doesn't.
</Callout>
