...
<Compare verdict="match" title="Same problem, same failure mode">
  <Side label="Prompt-based">
...
<KeyPoints title="The unit state machine, in four rules">
...
<TickSequence title="The cursor moves back" caption="Feedback on an earlier stage preempts forward progress, then the earlier stage opens new work.">
  <TickCard
    ...
    action="Feedback Awaiting Fix-Hat"
    dispatch="A small classifier reads the new feedback on the design stage, decides the fix needs a brand-new unit, writes that unit on design, and advances the feedback."
  >
