...
It doesn't checkout. It reads state from each stage branch without checking it out. The dashboard aggregates stage state across all the stage branches in one pass, and you see the whole intent's status regardless of which branch you happen to be sitting on.
