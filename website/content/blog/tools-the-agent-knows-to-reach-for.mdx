...
A unit without `applicable_skills` skips the block entirely. No empty header, no scan cost — `listInstalledSkills()` is gated behind `if (unitApplicableSkills.length > 0)` in `start_unit.ts:107`. The elaborator is told, in `elaborate.ts:131`, to be selective: *only annotate when there is clear relevance — don't bloat every unit*. Most units don't get annotation, and the prompt for those units stays exactly the same as before.
