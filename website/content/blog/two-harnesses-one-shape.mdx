...
<Compare verdict="match" title="State handoff through files">
  ...
  <Side label="H·AI·K·U">
    Our whole substrate is files: `.haiku/intents/<slug>/units/*.md`, `feedback/FB-NN.md`, `intent.md`. v4 dropped `state.json` and made outputs-on-disk the only signal the cursor reads.
  </Side>
</Compare>
...
<Compare verdict="match" title="Active runtime validation">
  ...
  <Side label="H·AI·K·U">
    `haiku_view` returns a URL (boot mode for runnable apps; viewer mode for static artifacts via the SPA artifact-browser with specialized viewers for KiCanvas, Tracespace, model-viewer, tscircuit). Per-studio `runtime-verifier.md` mandates hand it to the bundled headless Playwright MCP and save proof PNGs to `[intent]/[stages/<stage>/]proof/*.png`.
  </Side>
</Compare>
...
<Compare verdict="match" title="Log-driven prompt tuning">
  ...
  <Side label="H·AI·K·U">
    Default-on reflection loop. `record_observations` fires at each stage close (free-form `observations.md` capturing out-of-band churn). `record_reflection` fires at intent close: writes a synthesized `reflection.md`, lands cascade-fixable findings as overlays directly under `.haiku/...`, and reports engine-class findings via `haiku_report` with the stable `[autotune engine-class]` prefix. All in one `autotune:`-prefixed commit; PR review is the human gate.
  </Side>
</Compare>
