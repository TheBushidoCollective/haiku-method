...
|-------------|-------------|
| `/ai-dlc:elaborate` | `/haiku:haiku-start` + `/haiku:haiku-pickup` |
| `/ai-dlc:execute` | `/haiku:haiku-pickup` |
...
- **In-progress intents** — Continue using `/haiku:haiku-pickup` which reads from both old and new paths.
...
2. **Workflow selection during elaboration** — Replaced by studio selection during `/haiku:haiku-start`.
3. **`/ai-dlc:elaborate`** — Deprecated. Use `/haiku:haiku-start` to create intents, then `/haiku:haiku-pickup` to execute stages.
