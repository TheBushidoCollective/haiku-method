...
- **`/haiku:haiku-change-mode`**, **`/haiku:haiku-reflect`**, and a handful of operational commands (capacity, archive, etc.) round out the surface. To rewind to an earlier stage there is no separate slash command — file a stage_revisit feedback at the target stage via `haiku_feedback({ stage: "<target>", resolution: "stage_revisit" })` and call `haiku_run_next`. The cursor's Track B walks open feedback by file location and reroutes through the target stage; the engine's only verb is still `haiku_run_next`.
